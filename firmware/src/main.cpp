...
void audioDataCallback(const uint8_t *data, uint32_t len) {
  uint32_t sample_count =
      len / 4; // 2 channels * 16-bit (2 bytes) = 4 bytes per frame
...
void handleSerialCommands() {
  ...
  if (cmd.equalsIgnoreCase("status")) {
    Serial.println("--- AuraForge 50X Enterprise System Diagnostics ---");
    Serial.printf(
        "ESP32 Chip Rev: %d | CPU Freq: %d MHz | Silicon Temp: %.1f°C\n",
        ESP.getChipRevision(), ESP.getCpuFreqMHz(),
        g_hardwareMonitor.getTemperatureC());
    Serial.printf("Thermal Governor: %s | Attenuation: %.1f dB\n",
                  g_hardwareMonitor.getGovernorState() == GOVERNOR_NORMAL
                      ? "NORMAL (100% Gain)"
                      : "THROTTLED",
                  g_hardwareMonitor.getTelemetry().thermalAttenuation_dB);
    Serial.printf("Free Heap: %d bytes | Free Sketch Space: %d bytes\n",
                  ESP.getFreeHeap(), ESP.getFreeSketchSpace());
    Serial.printf("Wi-Fi STA: %s (IP: %s) | Wi-Fi AP: %s (IP: %s)\n",
                  g_wifiManager.isConnectedSTA() ? "CONNECTED" : "OFFLINE",
                  g_wifiManager.getLocalIp().c_str(),
                  g_wifiManager.isAPActive() ? "ACTIVE" : "DISABLED",
                  g_wifiManager.getApIp().c_str());
    Serial.printf("CSI Radar Presence: State %d | Motion: %.2f\n",
                  (int)g_csiRadarEngine.getPresenceState(),
                  g_csiRadarEngine.getTelemetry().motionIndex);
    Serial.printf("TPA3116D2 FAULT Pin: %s | Output: %s\n",
                  digitalRead(PIN_AMP_FAULT) == HIGH ? "NORMAL (OK)"
                                                     : "FAULT DETECTED",
                  g_hardwareMonitor.isAmplifierEnabled() ? "ACTIVE" : "MUTED");
...
void setup() {
  ...
  // 2. Initialize Hardware Protection & Power Monitor (Closed-Loop Thermal
  // Governor)
  g_hardwareMonitor.begin();

  // 3. Initialize 10-Band Parametric Equalizer, 128-Pt Radix-2 FFT & Audio
  // Engine
  g_dspEngine.begin();
  g_dspEngine.updateFromConfig(g_configManager.getConfig());
  ...
  // 4b. Initialize Wi-Fi CSI Motion Sensing & Presence State Machine
  g_csiRadarEngine.begin();

  // 5. Initialize Async HTTP Server, Dual-Bank OTA & WebSocket Telemetry
  ...
  Serial.printf(
      "[AuraForge 50X] Initializing Audio Engine & Bluetooth '%s'...\n",
