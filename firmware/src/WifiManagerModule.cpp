...
void WifiManagerModule::begin(const SystemConfig &config) {
  ...
  if (strlen(config.wifiStaSsid) > 0) {
    connectStation(config.wifiStaSsid, config.wifiStaPass, config.useStaticIp,
...
void WifiManagerModule::startAccessPoint(const char *apSsid,
                                         const char *apPass) {
  ...
  if (res) {
    ...
    // Start Captive Portal DNS server (redirects all domain queries to
    // 192.168.4.1)
...
bool WifiManagerModule::connectStation(const char *ssid, const char *pass,
                                       bool staticIp, const char *ip,
                                       const char *gw, const char *sub) {
  if (strlen(ssid) == 0)
    return false;
