 ...
 * @brief AuraForge 50X - Async Web Server, WebSocket Telemetry, Dual-Bank OTA &
 * REST API Engine
...
void WebServerModule::begin() {
  ...
  setupRoutes();
  server.begin();
  Serial.println("[Web Server] Async HTTP Server & WebSocket telemetry "
...
void WebServerModule::handleWebSocketMessage(void *arg, uint8_t *data,
                                             size_t len,
    ...
    if (!err) {
      if (doc.containsKey("action")) {
        ...
        if (strcmp(act, "reboot") == 0) {
          ...
          g_configManager.saveUserPreset(slot, name,
                                         g_configManager.getConfig().eqGains);
        } else if (strcmp(act, "loadPreset") == 0) {
          uint8_t slot = doc["slot"] | 0;
          if (g_configManager.loadUserPreset(
                  slot, g_configManager.getConfig().eqGains)) {
            g_dspEngine.updateFromConfig(g_configManager.getConfig());
          }
          ...
          snprintf(resp, sizeof(resp),
                   "{\"type\":\"csi_test_result\",\"success\":%s}",
                   ok ? "true" : "false");
          if (client)
            client->text(resp);
        }
        ...
        if (g_configManager.updateFromJson(doc)) {
          ...
          g_csiRadarEngine.setEnabled(cfg.csiStreamingEnabled ||
                                      cfg.csiPresenceAutomation);
          g_csiRadarEngine.setUdpStream(cfg.csiStreamingEnabled, cfg.csiPcIp,
                                        cfg.csiPcPort, cfg.csiStreamRateHz);
        }
      }
    }
  }
...
void WebServerModule::setupRoutes() {
  ...
  // POST /api/v1/status
  AsyncCallbackJsonWebHandler *statusPostHandler =
  ...
  server.on("/api/v1/presets", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "application/json",
                  g_configManager.getUserPresetsJson());
  });

  // POST /api/v1/presets/save
  AsyncCallbackJsonWebHandler *savePresetHandler =
      new AsyncCallbackJsonWebHandler(
          "/api/v1/presets/save",
          [](AsyncWebServerRequest *request, JsonVariant &json) {
            ...
            bool ok = g_configManager.saveUserPreset(
                slot, name, g_configManager.getConfig().eqGains);
  ...
  AsyncCallbackJsonWebHandler *loadPresetHandler =
      new AsyncCallbackJsonWebHandler(
          "/api/v1/presets/load",
          [](AsyncWebServerRequest *request, JsonVariant &json) {
            JsonObject obj = json.as<JsonObject>();
            uint8_t slot = obj["slot"] | 0;
            bool ok = g_configManager.loadUserPreset(
                slot, g_configManager.getConfig().eqGains);
            if (ok) {
              g_dspEngine.updateFromConfig(g_configManager.getConfig());
            }
  ...
  // POST /api/v1/eq
  AsyncCallbackJsonWebHandler *eqPostHandler = new AsyncCallbackJsonWebHandler(
      "/api/v1/eq", [](AsyncWebServerRequest *request, JsonVariant &json) {
        JsonObject jsonObj = json.as<JsonObject>();
  ...
  server.on("/api/v1/thermal", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(200, "application/json",
                  g_hardwareMonitor.getTelemetryJson());
  });
  ...
  AsyncCallbackJsonWebHandler *genHandler = new AsyncCallbackJsonWebHandler(
      "/api/v1/audio/generator",
      [](AsyncWebServerRequest *request, JsonVariant &json) {
        JsonObject obj = json.as<JsonObject>();
        int mode = obj["mode"] | 0;
  ...
  server.on(
      "/api/v1/system/reboot", HTTP_POST, [](AsyncWebServerRequest *request) {
  ...
  server.on("/api/v1/system/reset", HTTP_POST,
  ...
  server.on(
      ...
      [](AsyncWebServerRequest *request) {
        ...
        AsyncWebServerResponse *response =
            request->beginResponse(200, "text/plain", failure ? "FAIL" : "OK");
        response->addHeader("Connection", "close");
        request->send(response);
        if (!failure) {
          Serial.println("[OTA Flasher] Swap bank complete. Rebooting into new "
                         "firmware...");
          delay(500);
          ESP.restart();
        }
      ...
      [this](AsyncWebServerRequest *request, String filename, size_t index,
        ...
        if (!index) {
          const esp_partition_t *next = esp_ota_get_next_update_partition(NULL);
          Serial.printf(
              "[OTA Flasher] Flashing image '%s' to target partition '%s'...\n",
        ...
        if (!Update.hasError()) {
          if (Update.write(data, len) != len) {
            ...
            if (request->contentLength() > 0) {
              uint8_t progress =
                  (uint8_t)(((index + len) * 100) / request->contentLength());
              char progressMsg[64];
              snprintf(progressMsg, sizeof(progressMsg),
                       "{\"type\":\"ota_progress\",\"progress\":%u}", progress);
        ...
        if (final) {
          if (Update.end(true)) {
            Serial.printf(
                "[OTA Flasher] Dual-Bank Update Success: %u bytes verified.\n",
                index + len);
...
void WebServerModule::update() {
  ...
  if (now - lastWsBroadcast >=
      60) { // High frame-rate telemetry broadcast (~16 FPS)
    lastWsBroadcast = now;
    broadcastTelemetry();
  }
}
