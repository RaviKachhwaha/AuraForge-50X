 ...
 * Fault Protection & Power Monitor
...
enum ThermalGovernorState {
  ...
  GOVERNOR_CRITICAL    // >= 85°C: -6.0 dB digital attenuation / protective
                       // foldback
};
...
public:
  ...
  ThermalGovernorState getGovernorState() const {
    return telemetry.governorState;
  }
  float getThermalGainMultiplier() const {
    return telemetry.thermalGainMultiplier;
  }
